...
- 新增 `server/config.py` 统一加载运行参数，默认读取 `configs/default.yaml`，并支持通过环境变量（前缀 `AVATAR_`）覆盖，例如：
  - `AVATAR_CAMERA_INDICES=1,2` 指定摄像头枚举顺序；
  - `AVATAR_AUDIO_SAMPLE_RATE=44100` 自定义音频采样率；
  - `AVATAR_TTS_MIN_INTERVAL=3.5` 调整 TTS 触发节流。
- 配置加载后会在启动日志中输出完整字典，并自动确保音频缓存目录存在。
- `GET /config` 和 `GET /health` 接口会返回当前生效的配置摘要，方便前端或监控脚本读取。

## 🖥️ Web 客户端更新

- 前端样式重构为独立的 `style.css` + `app.js`，布局保持多模块仪表盘，同时提升卡片对比度与响应式体验。
- 摄像头卡片增加状态徽章：自动轮询 `/health`，提示摄像头是否就绪，以及模型文件是否加载成功。
- WebSocket 客户端实现指数退避重连机制，断线后会提示“尝试重连”并自动恢复，网络状态同步展示在“状态与回复”区域。
- 统一使用当前页面的协议与域名拼接 API/WS 地址，摆脱硬编码的 `127.0.0.1`，便于部署在同一局域网或 HTTPS 环境。
- 音频播放与 viseme 渲染逻辑模块化整理，音频获取失败时会在界面提示具体错误，方便排查。
...
- **数字人嘴型同步**：TTS 输出同时返回 viseme 序列（能量轨迹 + FPS），前端数字人面板可实时查看口型驱动数据。
- **对话仪表盘**：Web 客户端加入数字人面板、策略来源、触发原因、检索线索以及对话时间轴，方便观察策略演化。
